  ...
  if (!modal) {
    ...
    modal.innerHTML = `
      <div class="modal-box" style="max-width:420px; padding:2rem; position:relative;">
        ...
        <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:1rem;color:#999;"><div style="flex:1;height:1px;background:#ddd;"></div>або<div style="flex:1;height:1px;background:#ddd;"></div></div>
        <div id="authNameWrap" style="display:none;margin-bottom:0.75rem;"><input id="authName" type="text" placeholder="Ім'я" style="width:100%;padding:0.7rem 1rem;border-radius:8px;border:1px solid #ddd;font-size:1rem;box-sizing:border-box;background:var(--input-bg,#f5f5f5);color:inherit;"></div>
        <div style="margin-bottom:0.75rem;"><input id="authEmail" type="email" placeholder="Email" style="width:100%;padding:0.7rem 1rem;border-radius:8px;border:1px solid #ddd;font-size:1rem;box-sizing:border-box;background:var(--input-bg,#f5f5f5);color:inherit;"></div>
        <div style="margin-bottom:1.25rem;"><input id="authPassword" type="password" placeholder="Пароль" style="width:100%;padding:0.7rem 1rem;border-radius:8px;border:1px solid #ddd;font-size:1rem;box-sizing:border-box;background:var(--input-bg,#f5f5f5);color:inherit;"></div>
        <button id="authSubmitBtn" onclick="window.authEmailLogin()" style="width:100%;padding:0.75rem;border-radius:8px;border:none;background:#ffcc00;color:#333;font-size:1rem;font-weight:700;cursor:pointer;">Увійти</button>
      </div>
    `;
...
async function openProfileModal() {
  ...
  if (profileCache) {
    renderProfileContent(profileCache.bookmarks, profileCache.history);
    refreshProfileData(); // фонове оновлення
  } else {
...
function renderProfileContent(bookmarks, history) {
  const profileBody = document.getElementById('profileBody');
  if (!profileBody) return;
  ...
  profileBody.innerHTML = `
    <div style="display:flex;align-items:center;gap:1rem;padding:1rem 0 1.5rem;border-bottom:1px solid var(--border,#eee);margin-bottom:1.5rem;">
      ${user.photoURL ? `<img src="${user.photoURL}" style="width:60px;height:60px;border-radius:50%;object-fit:cover;">` : `<div style="width:60px;height:60px;border-radius:50%;background:#ffcc00;display:flex;align-items:center;justify-content:center;font-size:1.8rem;"><i class="fas fa-user"></i></div>`}
      <div style="flex:1;">
      ...
      <button onclick="window.authSignOut()" style="padding:0.4rem 0.9rem;border-radius:8px;border:1px solid #ddd;background:none;cursor:pointer;font-size:0.85rem;color:inherit;"><i class="fas fa-sign-out-alt"></i> Вийти</button>
    </div>
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.75rem;margin-bottom:1.5rem;">
      <div style="background:rgba(255,204,0,0.15);border-radius:10px;padding:0.9rem;text-align:center;"><div style="font-size:1.6rem;font-weight:700;color:#e6b800;">${bookmarks.length}</div><div style="font-size:0.75rem;color:#888;">Обране</div></div>
      <div style="background:rgba(0,0,0,0.05);border-radius:10px;padding:0.9rem;text-align:center;"><div style="font-size:1.6rem;font-weight:700;">${history.length}</div><div style="font-size:0.75rem;color:#888;">Переглянуто</div></div>
      <div style="background:rgba(0,0,0,0.05);border-radius:10px;padding:0.9rem;text-align:center;"><div style="font-size:1.6rem;font-weight:700;">★</div><div style="font-size:0.75rem;color:#888;">Рейтинг</div></div>
    </div>
  ...
  return `<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(100px,1fr));gap:0.75rem;">
    ${list.slice(0, 24).map(item => `
      <div class="profile-card" data-url="${item.url}" style="cursor:pointer;border-radius:10px;overflow:hidden;background:var(--card-bg,#f5f5f5);">
        <img src="${item.image_url || item.images?.jpg?.large_image_url}" alt="${item.title}" loading="lazy" style="width:100%;aspect-ratio:2/3;object-fit:cover;display:block;">
...
window.switchProfileTab = function (btn, tab) {
  document.querySelectorAll('.profile-tab').forEach(b => { b.style.background = 'transparent'; b.style.color = 'inherit'; });
...
// ---------- Detail Modal (миттєвий рендер + ліниве довантаження джерел) ----------
...
function renderDetailBaseUI(anime, isBookmarked, savedProgress) {
  const modalBody = document.getElementById('modalBody');
  if (!modalBody) return;
  ...
  modalBody.innerHTML = `
    ...
    <div id="sourcesArea" style="margin-top:1.5rem;">
      ${!anime._sourcesLoaded ? '<div style="text-align:center;padding:1rem;"><i class="fas fa-spinner fa-pulse"></i> Завантаження озвучок...</div>' : ''}
    </div>
  `;
