...
const Storage = {
  async getBookmarks() {
    if (window.currentUser) return await cloudGetBookmarks();
    return JSON.parse(localStorage.getItem('mono_anime_bookmarks') || '[]');
  },
  async saveBookmarks(arr) {
    if (window.currentUser) await cloudSaveBookmarks(arr);
    else localStorage.setItem('mono_anime_bookmarks', JSON.stringify(arr));
  },
  async getHistory() {
    if (window.currentUser) return await cloudGetHistory();
    return JSON.parse(localStorage.getItem('mono_anime_history') || '[]');
  },
  async addHistory(anime) {
    if (!anime || !anime.mal_id) return;
    if (window.currentUser) { await cloudAddHistory(anime); return; }
    ...
    filtered.unshift({
      ...
      url: anime.url || '', score: anime.score, year: anime.year,
      timestamp: Date.now()
    });
    localStorage.setItem('mono_anime_history', JSON.stringify(filtered.slice(0, 50)));
  },
  async clearHistory() {
    if (window.currentUser) await cloudClearHistory();
    else localStorage.setItem('mono_anime_history', '[]');
  },
